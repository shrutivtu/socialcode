        ...
        try {
            ...
            const processData = (arr: any) => {
                const res = arr.map((item: any) => {
                    const { name, html_url, forks_count, topics, visibility, open_issues, watchers } = item;
                    return{
                        name: name,
                        htmlUrl: html_url,
    ...
    return (
        <main className="w-4/5 my-0 mr-auto	 ml-auto pt-10 mb-2">
            <div className="w-9/10 my-0 mx-auto mb-4 flex items-center justify-center relative">
                ...
                />
                <button onClick={handleSubmit} className="border border-white-400 px-4 py-2 rounded hover:border-white-900 mr-2">Go</button>
                <button className="border border-white-400 px-4 py-2 rounded hover:border-white-900">+ Add Filter</button>
